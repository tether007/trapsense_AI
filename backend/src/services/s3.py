...
AWS_ACCESS_KEY_ID = os.getenv("AWS_ACCESS_KEY_ID") or os.getenv("ACCESS_KEY") or os.getenv("AWS_ACCESS_KEY")
AWS_SECRET_ACCESS_KEY = os.getenv("AWS_SECRET_ACCESS_KEY") or os.getenv("SECRET_ACCESS_KEY") or os.getenv("AWS_SECRET")
...
def upload_fileobj_to_s3(fileobj, object_name: str, content_type: str = None):
    ...
    if s3_client is None:
        raise RuntimeError("S3 credentials or bucket not configured (AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY/AWS_S3_BUCKET)")
    try:
        # fileobj may be a SpooledTemporaryFile or similar; ensure it's at start
        try:
            fileobj.seek(0)
        except Exception:
            pass
